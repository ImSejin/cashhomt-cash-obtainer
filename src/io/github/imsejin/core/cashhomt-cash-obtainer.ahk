...
; 로그 파일명
global logFileName := DISTRIBUTION_PATH . "log\execution-" . A_Now . ".log"
...
Start: 	; 시작버튼
	...
	; 로그를 출력한다
	Utils.logger("set values: {name: " . varNm . ", index: " . varIdx . "}", logFileName)
	...
	; 로그를 출력한다
	Utils.logger("start: the program", logFileName)
	...
	loop
	{
		...
		{
			; 로그를 출력한다
			Utils.logger("stop: the program", logFileName)
		...
		if (canGet)
		{
			...
			; 로그를 출력한다
			Utils.logger("obtain: cash", logFileName)
		}
		else
		{
			...
			{
				; 로그를 출력한다
				Utils.logger("wait: ad page (initial)", logFileName)
				...
				{
					; 로그를 출력한다
					Utils.logger("wait: ad page (repeated)", logFileName)
					sleep, 750
				...
				; 로그를 출력한다
				Utils.logger("close: ad page", logFileName)
			}
			...
			if (Utils.searchImage(IMG_GO_TO_GETTING_PAGE, noxWinHandle, x, y))
			{
				...
				; 로그를 출력한다
				Utils.logger("move: main page -> obtaining cash page", logFileName)
			}
			else if (Utils.searchImage(IMG_APP_ICON, noxWinHandle, x, y))
			{
				...
				; 로그를 출력한다
				Utils.logger("move: outside the app -> obtaining cash page", logFileName)
			}
			else if (Utils.searchImage(IMG_FAILED_TO_GET, noxWinHandle, x, y))
			{
				...
				; 로그를 출력한다
				Utils.logger("move: failed modal -> obtaining cash page", logFileName)
			}
			else if (Utils.searchImage(IMG_FINISH, noxWinHandle, x, y))
			{
				; 로그를 출력한다
				Utils.logger("finish: obtaining cash", logFileName)
...
GuiClose:
	; 로그를 출력한다
	Utils.logger("exit: the program", logFileName)
